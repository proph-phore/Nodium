...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test x$use_hardening != xno; then
    BITCOIN_QT_CHECK([
    AC_MSG_CHECKING(whether -fXN can be used with this Qt config)
    TEMP_CPPFLAGS=$CPPFLAGS
    TEMP_CXXFLAGS=$CXXFLAGS
    CPPFLAGS="$QT_INCLUDES $CPPFLAGS"
    CXXFLAGS="$XN_FLAGS $CXXFLAGS"
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[#include <QtCore/qconfig.h>]],
      ...
      [ AC_MSG_RESULT(yes); QT_XN_FLAGS=$XN_FLAGS ],
      [ AC_MSG_RESULT(no); QT_XN_FLAGS=$PIC_FLAGS]
    )
    CPPFLAGS=$TEMP_CPPFLAGS
    CXXFLAGS=$TEMP_CXXFLAGS
    ])
    ...
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[#include <QtCore/qconfig.h>]],
      ...
      [ AC_MSG_RESULT(no)],
      [ AC_MSG_RESULT(yes); QT_XN_FLAGS=$PIC_FLAGS]
    )
    CPPFLAGS=$TEMP_CPPFLAGS
    ])
  fi
  ...
  BITCOIN_QT_PATH_PROGS([LUPDATE], [XNdate-qt${bitcoin_qt_got_major_vers} XNdate${bitcoin_qt_got_major_vers} XNdate],$qt_bin_path, yes)
  ...
  BITCOIN_QT_CHECK([
    ...
    if test x$LUPDATE = x; then
      AC_MSG_WARN("XNdate is required to update qt translations")
    fi
  ],[
    bitcoin_enable_qt=no
  ])
